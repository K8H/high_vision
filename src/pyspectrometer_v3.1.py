    ...
    def __init__(self, args, window, window_title, video_source=0):
        ...
        def snapshot():
            ...
            if ret:
                ...
                jpg_snapshot = os.path.join(os.path.join(data_dir, 'img'), "spectrum-" + now + ".jpg")
                csv_snapshot = os.path.join(os.path.join(data_dir, 'csv'), now + '.csv')

                cv2.imwrite(jpg_snapshot, cv2.cvtColor(graphdata[0], cv2.COLOR_RGB2BGR))
                # print(graphdata[1]) #wavelengths
                # print(graphdata[2]) #intensities
                f = open(csv_snapshot, 'w')
